...
static esp_err_t _bf20a6_prepare_ctrl_pins(void)
{
    ...
    return gpba02b_write_io_level(CAM_IO_LIGHT_PORT, CAM_IO_LIGHT_PIN, 0U);
}
...
static esp_err_t _bf20a6_power_cycle(void)
{
    ...
    ret = gpba02b_write_io_level(CAM_IO_RESET_PORT, CAM_IO_RESET_PIN, 0U);
    if (ret != ESP_OK) {
        return ret;
    }

    ret = gpba02b_write_io_level(CAM_IO_PWDN_PORT, CAM_IO_PWDN_PIN, 1U);
    if (ret != ESP_OK) {
        return ret;
    }
    delay_ms(20);

    ret = gpba02b_write_io_level(CAM_IO_PWDN_PORT, CAM_IO_PWDN_PIN, 0U);
    if (ret != ESP_OK) {
        return ret;
    }
    delay_ms(20);

    ret = gpba02b_write_io_level(CAM_IO_RESET_PORT, CAM_IO_RESET_PIN, 1U);
    if (ret != ESP_OK) {
        return ret;
    }
    delay_ms(30);
...
{
    (void)gpba02b_write_io_level(CAM_IO_LIGHT_PORT, CAM_IO_LIGHT_PIN, 0U);
    (void)gpba02b_write_io_level(CAM_IO_RESET_PORT, CAM_IO_RESET_PIN, 0U);
    (void)gpba02b_write_io_level(CAM_IO_PWDN_PORT, CAM_IO_PWDN_PIN, 1U);
}
...
static esp_err_t _bf20a6_apply_sensor_fixed_profile(void)
{
    ...
    ESP_LOGI(
        TAG,
        "fixed sensor cfg idx=%u yuv_seq=%u href=%u vclk_b=%u vclk_a=%u div_inv=%u "
        "test=0x%02X hm=%u vf=%u",
        (unsigned)slot,
        (unsigned)yuv_seq,
...
static esp_err_t _bf20a6_apply_clock_fixed_profile(void)
{
    ...
    if (!profile->sensor_div) {
        ESP_LOGI(TAG, "fixed clk cfg idx=%u use_div=0 (skip E3/F0)", (unsigned)slot);
        return ESP_OK;
    }
...
esp_err_t bf20a6_cam_set_light(bool enable)
{
    ...
    return gpba02b_write_io_level(CAM_IO_LIGHT_PORT, CAM_IO_LIGHT_PIN, enable ? 1U : 0U);
}
...
esp_err_t bf20a6_cam_open(void)
{
    camera_config_t cfg = {
        .pin_pwdn = -1,
        .pin_reset = -1,
        .pin_xclk = CAM_XCLK_EXTERNAL_OSC ? -1 : CAM_IO_XCLK,
