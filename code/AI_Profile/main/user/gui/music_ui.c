...
static void _music_ui_task(void* param) {
    ...
    while (1) {
        btn_status_e btn_val =
            button_scan_state(&runtime->button_scan, MUSIC_UI_TASK_PERIOD_MS);
        if ((btn_val == Btn_Both_Click) && (runtime->home_cb != NULL)) {
            runtime->home_cb(runtime->home_user_ctx);
        }
        delay_ms(MUSIC_UI_TASK_PERIOD_MS);
    }
}

/*
 * brief : Create the music page root screen object.
 * input : lcd_w - LCD width; lcd_h - LCD height.
 * output: Created LVGL screen object.
 * type  : public
 */
lv_obj_t* music_create_screen(lv_obj_t* parent, lv_coord_t area_w, lv_coord_t area_h,
    ...
    if (s_music_runtime.task_handle != NULL) {
        ...
        s_music_runtime.task_handle = NULL;
    }

    s_music_runtime.home_cb = home_cb;
    s_music_runtime.home_user_ctx = home_user_ctx;
    s_music_runtime.button_scan = (btn_scan_s){0};
    ...
    BaseType_t task_ok =
        xTaskCreate(_music_ui_task, "music_ui", MUSIC_UI_TASK_STACK_SIZE, &s_music_runtime, 5,
                    &s_music_runtime.task_handle);
    if (task_ok != pdPASS) {
        s_music_runtime.task_handle = NULL;
        lv_obj_del(screen);
...
void music_destroy_screen(lv_obj_t* screen) {
    ...
    s_music_runtime.home_cb = NULL;
    s_music_runtime.home_user_ctx = NULL;
    s_music_runtime.button_scan = (btn_scan_s){0};
