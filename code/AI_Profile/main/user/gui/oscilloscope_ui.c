...
static scope_ui_runtime_s s_scope_runtime;

/*
 * brief : _scope_ui_task.
 * input : see parameters.
 * output: none.
 * type  : private
 */
static void _scope_ui_task(void* param) {
    scope_ui_runtime_s* runtime = (scope_ui_runtime_s*)param;

    while (1) {
        btn_status_e btn_val =
            button_scan_state(&runtime->button_scan, SCOPE_UI_TASK_PERIOD_MS);
        if ((btn_val == Btn_Both_Click) && (runtime->home_cb != NULL)) {
            runtime->home_cb(runtime->home_user_ctx);
        }
        delay_ms(SCOPE_UI_TASK_PERIOD_MS);
    }
}

/*
 * brief : Create the scope page root screen object.
...
lv_obj_t* scope_create_screen(lv_obj_t* parent, lv_coord_t area_w, lv_coord_t area_h,
    ...
    s_scope_runtime.home_cb = home_cb;
    s_scope_runtime.home_user_ctx = home_user_ctx;
    s_scope_runtime.button_scan = (btn_scan_s){0};
    ...
    BaseType_t task_ok =
        xTaskCreate(_scope_ui_task, "scope_ui", SCOPE_UI_TASK_STACK_SIZE, &s_scope_runtime, 5,
                    &s_scope_runtime.task_handle);
    if (task_ok != pdPASS) {
        s_scope_runtime.task_handle = NULL;
        lv_obj_del(screen);
...
void scope_destroy_screen(lv_obj_t* screen) {
    ...
    s_scope_runtime.home_cb = NULL;
    s_scope_runtime.home_user_ctx = NULL;
    s_scope_runtime.button_scan = (btn_scan_s){0};
