...
static void _print_ui_task(void* param) {
    ...
    while (1) {
        btn_status_e btn_val =
            button_scan_state(&runtime->button_scan, PRINT_UI_TASK_PERIOD_MS);
...
lv_obj_t* print_create_screen(lv_obj_t* parent, lv_coord_t area_w, lv_coord_t area_h,
    ...
    BaseType_t task_ok =
        xTaskCreate(_print_ui_task, "print_ui", PRINT_UI_TASK_STACK_SIZE, &s_print_runtime, 5,
...
void print_destroy_screen(lv_obj_t* screen) {
    ...
    s_print_runtime.home_cb = NULL;
    s_print_runtime.home_user_ctx = NULL;
    s_print_runtime.button_scan = (btn_scan_s){0};
