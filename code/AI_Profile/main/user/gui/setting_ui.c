...
static void _setting_ui_task(void* param) {
    ...
    while (1) {
        btn_status_e btn_val =
            button_scan_state(&runtime->button_scan, SETTING_UI_TASK_PERIOD_MS);
...
lv_obj_t* setting_create_screen(lv_obj_t* parent, lv_coord_t area_w, lv_coord_t area_h,
    ...
    BaseType_t task_ok =
        xTaskCreate(_setting_ui_task, "setting_ui", SETTING_UI_TASK_STACK_SIZE,
                &s_setting_runtime, 5, &s_setting_runtime.task_handle);
    if (task_ok != pdPASS) {
        s_setting_runtime.task_handle = NULL;
        lv_obj_del(screen);
