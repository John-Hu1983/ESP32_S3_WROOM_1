...
static void _power_lock(bool enable) {
...
static void _pwm_default(gpba02b_port_t port, uint8_t pin, gpba02b_pwm_freq_t fre,
                         uint8_t percent) {
...
void bsp_init_total(void) {
    ...
    _pwm_default(GPBA02B_PORT_C, 1, GPBA02B_PWM_FREQ_1343HZ_DIV32, 12);
    _button_gpio();
    bsp_lcd_reset_sequence();
    desktop_start();
    bg_start(desktop_get_cpu_label(), desktop_get_net_label());
}
