 ...
 */
static void *_pic_gif_alloc(size_t bytes)
{
    void *ptr;
 ...
 */
static bool _pic_gif_suffix_match_ci(const char *path, const char *suffix)
{
    size_t path_len;
    size_t suffix_len;
    size_t i;
...
/*
 * brief: Validate GIF signature.
 * input: data - file bytes; size - byte length.
 * output: true when GIF87a or GIF89a signature is present.
 */
static bool _pic_gif_is_magic_valid(const uint8_t *data, size_t size)
{
...
{
    return _pic_gif_suffix_match_ci(path, ".gif");
}

bool pic_gif_decoder_enabled(void)
{
#if defined(CONFIG_LV_USE_GIF) && CONFIG_LV_USE_GIF
    return true;
#else
    return false;
#endif
...
esp_err_t pic_gif_decode_item(pic_item_t *item)
{
    ...
    if ((item == NULL) || (item->path == NULL) || !pic_gif_path_is_match(item->path))
    {
    ...
    if (!_pic_gif_is_magic_valid(src_data, src_size))
    {
        free(src_data);
    ...
    if (res != LV_RES_OK)
    {
        ESP_LOGW(TAG, "gif decode open failed, path=%s", item->path);
        free(src_data);
        return ESP_ERR_NOT_SUPPORTED;
    }
    ...
    decoded_copy = (uint8_t *)_pic_gif_alloc((size_t)decoded_size);
