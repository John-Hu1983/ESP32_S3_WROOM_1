 ...
 */
static void *_pic_png_alloc(size_t bytes)
{
    void *ptr;
 ...
 */
static bool _pic_png_suffix_match_ci(const char *path, const char *suffix)
{
    size_t path_len;
    size_t suffix_len;
    size_t i;
...
{
    return _pic_png_suffix_match_ci(path, ".png");
}

bool pic_png_decoder_enabled(void)
{
#if defined(CONFIG_LV_USE_PNG) && CONFIG_LV_USE_PNG
    return true;
#else
    return false;
#endif
...
esp_err_t pic_png_decode_item(pic_item_t *item)
{
    ...
    if ((item == NULL) || (item->path == NULL) || !pic_png_path_is_match(item->path))
    {
    ...
    if (!_pic_png_is_magic_valid(src_data, src_size))
    {
        free(src_data);
    ...
    if (res != LV_RES_OK)
    {
        ESP_LOGW(TAG, "png decode open failed, path=%s", item->path);
        free(src_data);
        return ESP_ERR_NOT_SUPPORTED;
    }
    ...
    decoded_copy = (uint8_t *)_pic_png_alloc((size_t)decoded_size);
